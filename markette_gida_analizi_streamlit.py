...
def search_off_by_name(query):
    ...
    params = {"search_terms": query, "json": 1, "page_size": 6, "fields": "product_name,brands,code"}
    try:
        r = requests.get(url, params=params, timeout=8, headers={"User-Agent": "GidaAnalizWeb/2.1"})
...
st.caption("Web versiyonu • Cep telefonunda çalışır")

tab1, tab2, tab3, tab4 = st.tabs(["📋 Ürünler", "📡 OFF'tan Ekle", "🛒 Sepet", "📊 Analiz"])

with tab1:
    search = st.text_input("Ara", placeholder="süt, ekmek...")
    ...
    for _, row in filtered.iterrows():
        ...
        with col3:
            ...
            if st.button("Ekle", key=f"add{row['id']}"):
                add_to_sepet(row["id"], qty)
                st.success("Eklendi!")
                st.rerun()

with tab2:
    st.subheader("Open Food Facts")
    method = st.radio("Yöntem", ["Barkod", "İsimle Ara"])
    if method == "Barkod":
        bc = st.text_input("Barkod")
        if st.button("Getir") and bc:
            prod, err = fetch_product_from_off(bc)
            if err: st.error(err)
            else:
                st.success(prod["name"])
                pkg = st.number_input("Paket g", value=prod["package_g"])
        ...
        q = st.text_input("Ürün ismi")
        if st.button("Ara") and q:
            results = search_off_by_name(q)
            for p in results:
                if st.button(p.get("product_name", "Ürün"), key=p.get("code")):
                    bc = p.get("code")
                    prod, err = fetch_product_from_off(bc)
                    if prod:
                        st.session_state.temp_prod = prod
                        st.session_state.temp_bc = bc
                        st.rerun()
    ...
    if not st.session_state.sepet:
        ...
        for item in st.session_state.sepet:
            ...
            if prod:
                ...
                if st.button("Sil", key=f"del{item['id']}"):
                    remove_from_sepet(item["id"])
                    st.rerun()
        st.metric("Toplam", f"{sum(get_product_by_id(i['id'])['fiyat_tl']*i['miktar'] for i in st.session_state.sepet if get_product_by_id(i['id'])):.2f} ₺")
...
with tab4:
    ...
    if data:
        t = data["totals"]
        p = data["perc"]
        st.metric("Kalori", f"{t['kcal']:.0f}", f"{p['kcal']}%")
        fig, ax = plt.subplots()
        ax.pie([t["protein"]*4, t["karb"]*4, t["yag"]*9], labels=["Protein","Karb","Yağ"], autopct="%1.1f%%")
        st.pyplot(fig)
        if st.button("Rapor İndir"):
            df = pd.DataFrame(data["details"])
            st.download_button("CSV İndir", df.to_csv(index=False), "rapor.csv")
    else:
        st.info("Sepete ürün ekle")

st.caption("Başarılı denemeler!")